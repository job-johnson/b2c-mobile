    ...
    public function addToCartEvent($observer) {
       ...
       // echo "<pre>"; print_r($request->getParams());exit;
        ...
        if (!$request->getParam('in_cart') && !$request->getParam('is_checkout')) {
            ...
            //$count = $observer->getProduct()->getName()."<--->".."<--->";
            $_response = Mage::getModel('ajaxcart/response')
                    ->setProductName($observer->getProduct()->getName())
                    ->setMessage(Mage::helper('checkout')->__('%s was added into cart.', Mage::helper('checkout/cart')->getCart()->getItemsCount().$observer->getProduct()->getName()));
        ...
        if ($request->getParam('is_checkout')) {
            ...
            Mage::getSingleton('checkout/session')->setNoCartRedirect(true);
          //  $count = $observer->getProduct()->getName()."<--->".Mage::helper('checkout/cart')->getCart()->getItemsCount()."<--->";
            $_response = Mage::getModel('ajaxcart/response')
                    ->setProductName($observer->getProduct()->getName())
                    ->setMessage(Mage::helper('checkout')->__('%s was added into cart.', Mage::helper('checkout/cart')->getCart()->getItemsCount().$observer->getProduct()->getName()));
            $_response->send();
